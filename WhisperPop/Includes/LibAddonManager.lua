...
local function Addon_RegisterSlashCmd(self, ...)
	local UPPER_NAME = string.upper(self.name)

	for i = 1, select("#", ...) do
		local cmd = select(i, ...)
		if type(cmd) == "string" then
			if string.find(cmd, "/") ~= 1 then
				cmd = "/"..cmd
			end
	...
	SlashCmdList[UPPER_NAME] = function(text)
		if type(self.OnSlashCmd) == "function" then
			self:OnSlashCmd(string.trim(text)) -- The addon wants to process the slash command itself
		elseif type(self.OnClashCmd) == "function" then
			self:OnClashCmd(string.trim(text)) -- The addon wants to process the slash command itself
		else
...
local function Addon_RegisterBindingClick(self, button, name, text)
	...
	--local header = "BINDING_HEADER_"..string.upper(self.name).."_TITLE"
...
local function BCO_RegisterEventCallback(self, event, func, arg1)
	...
	table.insert(callbacks, { func = func, arg1 = arg1 })
end
...
local function Addon_CreateModule(self, key, dbType, ...)
	...
	table.insert(self[PRIVATE].modules, module)
...
local function PopupData_OnAccept(self, arg1, func)
	...
	local text = string.trim(editBox:GetText())
...
function lib:CreateAddon(name, addon)
	...
	table.insert(lib._addonList, addon)
	return addon
end
