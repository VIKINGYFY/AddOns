...
function addon:DecodeMessage(line)
	...
	local timestamp, text = string.match(line, "^[01]%[T(%d-)%](.*)")
	local formattedTime = timestamp and addon:GetFormattedTime(timestamp)
	if not formattedTime then
		formattedTime, text = string.match(line, "^[01](%[.-%])(.*)")
	end
	if not formattedTime then
		formattedTime = string.sub(line, 2, 17)
		text = string.sub(line, 18)
	end
...
function addon:ParseNameRealm(text)
	if type(text) == "string" then
		local _, _, name, realm = string.find(text, "(.+)%-(.+)")
...
function addon:IsBattleTag(name)
	if type(name) == "string" then
		local _, _, prefix, surfix = string.find(name, "(.+)#(%d+)$")
...
function addon:GetNewNames()
	...
	for i = 1, #self.db.history do
		local data = self.db.history[i]
		if data.new then
			table.insert(newNames, addon:GetDisplayName(data.name))
		end
	end
	return newNames
end
...
function addon:Delete(name)
	...
	if index and not data.protected then
		table.remove(self.db.history, index)
		self:BroadcastEvent("OnListUpdate")
	end
end
...
function addon:ProcessChatMsg(name, class, text, inform, bnid)
	...
	if index then
		if index > 1 then
			table.remove(self.db.history, index)
			table.insert(self.db.history, 1, data)
		end
	else
...
function addon:AddIgnore(pattern)
	...
	table.insert(self.IGNORED_MESSAGES, pattern)
end
