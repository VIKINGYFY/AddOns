...
local fontSize = 14  -- 承受/输出框架字体大小
local infoFont = 16  -- 信息框架字体大小
local fontType = STANDARD_TEXT_FONT  -- 字体路径："Fonts\\xxx.ttf"
...
local function GetFloatingIcon(iconType, spellID, isPet)
	...
	elseif iconType == "swing" then
		if isPet then
			...
			texture = C_Spell.GetSpellTexture(6603)
		end
	elseif iconType == "range" then
		texture = C_Spell.GetSpellTexture(75)
	elseif iconType == "env" then
		texture = "Interface\\Icons\\"..envTexture[spellID]
	end

	if not texture then
		texture = C_Spell.GetSpellTexture(195112)
	end
	...
	if ((isPlayer or isVehicle or isPet or noPlayer) and atTarget) or atPlayer then
		...
		if value then
			...
			elseif value.suffix == "FAILED" then
				...
				local intName = C_Spell.GetSpellName(extraSpellID)
				...
				if atTarget then
					...
					elseif sourceName then
						sourceName = string.split("-", sourceName)
