...
local LMFrame = CreateFrame("Frame", "LootMonitor", UIParent, "BackdropTemplate")
LMFrame:SetFrameStrata("HIGH")
LMFrame:SetClampedToScreen(true)
LMFrame:SetPoint("LEFT", 4, 0)

local LMFrame_Title = B.CreateFS(LMFrame, Button_Height-2, L["LootMonitor Title"], "class", "TOPLEFT", 10, -10)
local LMFrame_Info = B.CreateFS(LMFrame, Button_Height-2, L["LootMonitor Info"], "class", "BOTTOMRIGHT", -10, 10)
...
local function CreateLMFrame()
	...
	if B.CreateMF then B.CreateMF(LMFrame) end

	local LMFrame_Close = B.CreateButton(LMFrame, 18, 18, DB.closeTex)
