...
oUF.Tags.Methods["targetname"] = function(unit)
	...
	if UnitExists(unitTarget) then
		local tarClass = select(2, UnitClass(unitTarget))
		local tarName = UnitName(unitTarget)

		return B.HexRGB(oUF.colors.class[tarClass])..tarName
	end
end
oUF.Tags.Events["targetname"] = "UNIT_NAME_UPDATE UNIT_THREAT_LIST_UPDATE UNIT_THREAT_SITUATION_UPDATE UNIT_TARGET UNIT_TARGETABLE_CHANGED"
