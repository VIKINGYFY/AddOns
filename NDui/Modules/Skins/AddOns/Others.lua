...
function S:FriendGroups()
	...
	if FriendGroups_UpdateFriendButton then
		local function replaceButtonStatus(self, texture)
			...
			self.bg:Show()
			if string.find(texture, "PlusButton") then
				self:SetAtlas("Soulbinds_Collection_CategoryHeader_Collapse", true)
			elseif string.find(texture, "MinusButton") then
...
function S:SoulbindsTalents()
	if not C_AddOns.IsAddOnLoaded("SoulbindsTalents") then return end
...
function S:SoulshapeJournal()
	if not C_AddOns.IsAddOnLoaded("SoulshapeJournal") then return end
...
function S:ATT()
	if not C_AddOns.IsAddOnLoaded("AllTheThings") then return end
...
function S:TrinketMenu()
	if not C_AddOns.IsAddOnLoaded("TrinketMenu") then return end
...
function S:PSFJ()
	if not C_AddOns.IsAddOnLoaded("ProtoformSynthesisFieldJournal") then return end
...
function S:TLDR()
	if not C_AddOns.IsAddOnLoaded("TLDRMissions") then return end
...
function S:ManuscriptsJournal()
	if C_AddOns.IsAddOnLoaded("ManuscriptsJournal") then
		handleJournal()
	else
		local hooked
