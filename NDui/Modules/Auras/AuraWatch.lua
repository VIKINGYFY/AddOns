...
local function ConvertTable()
	...
	for i, v in pairs(C.AuraWatchList["ALL"]) do
		...
		elseif v.Name == "InternalCD" then
			...
			IntCD = v
			table.remove(C.AuraWatchList["ALL"], i)
		end
	end
end
...
local function BuildUnitIDTable()
	for _, VALUE in pairs(AuraList) do
		for _, value in pairs(VALUE.List) do
			...
			if flag then table.insert(UnitIDTable, value.UnitID) end
		end
	end
end

local function BuildCooldownTable()
	table.wipe(cooldownTable)
...
local function tooltipOnEnter(self)
	...
	elseif self.type == 2 then
		GameTooltip:SetHyperlink(select(2, C_Item.GetItemInfo(self.spellID)))
...
local function BuildAura()
	for key, value in pairs(AuraList) do
		...
		frameTable.Index = 1
		table.insert(FrameList, frameTable)
	end
end
		...
		for spellID in pairs(VALUE) do
			...
			if value then
				if value.SpellID then
					local name, icon = C_Spell.GetSpellName(value.SpellID), C_Spell.GetSpellTexture(value.SpellID)
				...
				elseif value.ItemID then
					...
					if start and duration > C.db["AuraWatch"]["MinCD"] then
						local name, _, _, _, _, _, _, _, _, icon = C_Item.GetItemInfo(value.ItemID)
						if group.Mode == "ICON" then name = nil end
...
function A:AuraWatch_SetupInt(intID, itemID, duration, unitID, guid, sourceName)
	...
	if itemID then
		...
		name, icon = C_Spell.GetSpellName(intID), C_Spell.GetSpellTexture(intID)
		frame.type = 1
		frame.spellID = intID
	end
...
function A:AuraWatch_UpdateInt(event, ...)
	...
	if event == "UNIT_SPELLCAST_SUCCEEDED" then
		...
		if value and value.CastSucceed and unit then
			...
			local isPassed
			if unitID == "all" and (unit == "player" or string.find(unit, "pet") or UnitInRaid(unit) or UnitInParty(unit) or not GetPlayerInfoByGUID(guid)) then
				isPassed = true
