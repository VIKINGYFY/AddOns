...
local offlineString = string.gsub(ERR_FRIEND_OFFLINE_S, "%%s", "")
...
local function buildFriendTable(num)
	...
	for i = 1, num do
		...
		if info and info.connected then
			...
			table.insert(friendTable, {info.name, info.level, class, info.area, status, info.notes})
		end
	end
	...
	for i = 1, num do
		...
		if accountInfo then
			...
			if isOnline and gameID then
				...
				table.insert(bnetTable, {i, accountName, charName, factionName, client, status, class, level, infoText, note, broadcastText, broadcastTime})
			end
		end
	end
...
local function buttonOnEnter(self)
	...
	if self.isBNet then
		GameTooltip:AddLine(L["BN"], 0,1,1)
		GameTooltip:AddLine(" ")
...
info.onEnter = function(self)
	...
	if totalOnline == 0 then
		...
		GameTooltip:ClearLines()
		GameTooltip:AddDoubleLine(FRIENDS_LIST, format("%s: %s / %s", GUILD_ONLINE_LABEL, totalOnline, totalFriends), 0,1,1, 0,1,1)
		GameTooltip:AddLine(" ")
