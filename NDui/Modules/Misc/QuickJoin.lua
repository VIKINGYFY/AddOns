	...
	if searchResultInfo then
		...
		if activityInfo then
			...
			if showScore then
				...
				local newName = B.ReplaceText(oldName)
				self.ActivityName:SetFormattedText(scoreFormat, TT.GetDungeonScore(showScore), newName)
...
function M:FixListingTaint() -- From PremadeGroupsFilter
	if C_AddOns.IsAddOnLoaded("PremadeGroupsFilter") then return end
