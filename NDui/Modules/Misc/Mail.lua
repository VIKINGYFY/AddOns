		...
		if itemAttached > 1 then
			...
			for itemID, count in pairs(inboxItems) do
				...
				if itemName then
					...
					GameTooltip:AddDoubleLine(" |T"..itemTexture..":12:12:0:0:64:64:5:59:5:59|t "..itemName, count, r,g,b)
				end
			end
			GameTooltip:Show()
		end
...
function M:MailBox_ContactList()
	...
	add:SetScript("OnClick", function()
		...
		if not string.find(text, "-") then text = text.."-"..DB.MyRealm end -- complete player realm name
...
function M:MailBox_CollectGold()
	if mailIndex > 0 then
		...
		C_Timer.After(timeToWait, M.MailBox_CollectGold)
	else
...
function M:MailBox_CollectAttachment()
	for i = 1, ATTACHMENTS_MAX_RECEIVE do
		...
		if attachmentButton:IsShown() then
			...
			C_Timer.After(timeToWait, M.MailBox_CollectAttachment)
			return
		end
	end
end
