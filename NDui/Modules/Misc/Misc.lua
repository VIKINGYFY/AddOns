...
function M:BlockStrangerInvite()
	B:RegisterEvent("PARTY_INVITE_REQUEST", function(_, _, _, _, _, _, _, guid)
		if C.db["Misc"]["BlockInvite"] and not (C_BattleNet.GetGameAccountInfoByGUID(guid) or C_FriendList.IsFriend(guid) or IsGuildMember(guid)) then
			DeclineGroup()
...
local guildInviteString = string.gsub(CHAT_GUILD_INVITE_SEND, HEADER_COLON, "")
...
function M:ToggleAddOnProfiler()
	...
	local bu = B.CreateCheckBox(AddonList, true)
	bu:SetPoint("BOTTOM", 0, 2)
	bu:SetChecked(not(not NDuiADB["AddOnProfiler"]))
	bu.text = B.CreateFS(bu, 14, L["CPU Usage"], "system", "LEFT", 30, 0)
