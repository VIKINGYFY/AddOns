...
function Bar:SendCurrentSpell(thisTime, spellID)
	...
	if charges and maxCharges then
		if charges ~= maxCharges then
			...
			C_ChatInfo.SendChatMessage(format(L["ChargesRemaining"], spellLink, charges, maxCharges, GetRemainTime(remain)), B.GetCurrentChannel())
		else
			C_ChatInfo.SendChatMessage(format(L["ChargesCompleted"], spellLink, charges, maxCharges), B.GetCurrentChannel())
		end
	else
		...
		if start and duration > 0 then
			...
			C_ChatInfo.SendChatMessage(format(L["CooldownRemaining"], spellLink, GetRemainTime(remain)), B.GetCurrentChannel())
		else
			C_ChatInfo.SendChatMessage(format(L["CooldownCompleted"], spellLink), B.GetCurrentChannel())
		end
	end
end
...
function Bar:SendCurrentItem(thisTime, itemID, itemLink, itemCount)
	...
	if start and duration > 0 then
		local remain = start + duration - thisTime
		C_ChatInfo.SendChatMessage(format(L["CooldownRemaining"], itemLink.." x"..itemCount, GetRemainTime(remain)), B.GetCurrentChannel())
	else
		C_ChatInfo.SendChatMessage(format(L["CooldownCompleted"], itemLink.." x"..itemCount), B.GetCurrentChannel())
	end
end
