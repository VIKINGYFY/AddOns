...
local function GetCurrentTime()
	local locTime = time()
	local realmTime = not GetCVarBool("timeMgrUseLocalTime") and C_DateAndTime.GetCurrentCalendarTime()
	if realmTime then
...
function module:UpdateChannelNames(text, ...)
	if string.find(text, INTERFACE_ACTION_BLOCKED) and not DB.isDeveloper then return end

	local r, g, b = ...
	if C.db["Chat"]["WhisperColor"] and string.find(text, L["Tell"].." |H[BN]*player.+%]") then
		r, g, b = r*.7, g*.7, b*.7
	end
	...
	if unitName and DB.Devs[unitName] then
		text = string.gsub(text, "(|Hplayer.+)", "|T"..DB.chatLogo..":12:24|t%1")
	end
	...
	if NDuiADB["TimestampFormat"] > 1 then
		...
		local oldTimeStamp = defaultTimestamp and string.gsub(BetterDate(defaultTimestamp, locTime), "%[([^]]*)%]", "%%[%1%%]")
		if oldTimeStamp then
			text = string.gsub(text, oldTimeStamp, "")
		end
...
function module:ChannelRename()
	...
	--online/offline info
	ERR_FRIEND_ONLINE_SS = string.gsub(ERR_FRIEND_ONLINE_SS, "%]%|h", "]|h|cff00C957")
	ERR_FRIEND_OFFLINE_S = string.gsub(ERR_FRIEND_OFFLINE_S, "%%s", "%%s|cffFF7F50")
