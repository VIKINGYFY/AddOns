...
function B:Decode(text)
	...
	for i = 1, #text do
		...
		if not (whitespace[byte] or byte == equals_byte) then
			...
			if not num then
				...
				error(format("Bad argument #1 to `Decode'. Received an invalid char: %q", string.sub(text, i, i)), 2)
			end
	...
	for i = 1, #t2, 4 do
		...
		t[#t+1] = string.char(a)
		if nilNum < 2 then t[#t+1] = string.char(b) end
		if nilNum < 1 then t[#t+1] = string.char(c) end
	end
