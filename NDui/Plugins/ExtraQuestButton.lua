...
ExtraQuestButton:RegisterForClicks("AnyDown", "AnyUp")
...
local function GetClosestQuestItem()
	...
	for i = 1, #tasksTable do
		...
		if questID and not C_QuestLog.IsWorldQuest(questID) and not QuestUtils_IsQuestWatched(questID) and GetTaskInfo(questID) then
			local distance, itemLink = GetQuestDistanceWithItem(questID)
			if distance and distance <= closestDistance then
				closestDistance = distance
				closestQuestItemLink = itemLink
