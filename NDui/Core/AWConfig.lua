...
local function CreatePanel()
	...
	local function SortBars(index)
		...
		for k in pairs(barTable[index]) do
			if (index < 10 and C.db["AuraWatchList"][index][k]) or (index == 10 and C.db["InternalCD"][k]) then
				...
				elseif num > 1 and num/2 ~= math.floor(num/2) then
					bar:SetPoint("TOPLEFT", 10, -10 - 35*onLeft)
					onLeft = onLeft + 1
				elseif num == 2 then
					bar:SetPoint("TOPLEFT", 295, -10)
				elseif num > 2 and num/2 == math.floor(num/2) then
	...
	local groups = {
		...
		L["Spell Cooldown"], -- 6 Spell Cooldown
	...
	for i, group in pairs(groups) do
		...
		add:SetScript("OnClick", function()
			if i < 10 then
				...
				if (typeID == "AuraID" or typeID == "SpellID") and not C_Spell.GetSpellName(spellID) then UIErrorsFrame:AddMessage(DB.InfoColor..L["Incorrect SpellID"]) return end
			...
			elseif i == 10 then
				...
				if intID and not C_Spell.GetSpellName(intID) then UIErrorsFrame:AddMessage(DB.InfoColor..L["Incorrect SpellID"]) return end
