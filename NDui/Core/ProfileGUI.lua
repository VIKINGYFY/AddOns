...
function G:ImportGUIData()
	...
	local title, version, _, class = string.split(":", options[1])
	...
	for i = 2, #options do
		...
		elseif string.find(value, "Color") and (arg1 == "r" or arg1 == "g" or arg1 == "b") then
			local colors = {select(3, string.split(":", option))}
		...
		elseif key == "AuraWatchList" then
			if value == "Switcher" then
				local index, state = select(3, string.split(":", option))
			...
			elseif value == "IgnoreSpells" then
				...
				local idType, spellID, unit, caster, stack, amount, timeless, combat, text, flash = select(4, string.split(":", option))
		...
		elseif booleanTable[value] then
			local results = {select(3, string.split(":", option))}
		...
		elseif key == "InternalCD" then
			local spellID, duration, indicator, unit, itemID = select(3, string.split(":", option))
			spellID = tonumber(spellID)
		...
		elseif key == "ACCOUNT" then
			...
			elseif value == "RaidDebuffs" then
				local instName, spellID, priority = select(3, string.split(":", option))
			...
			elseif value == "ContactList" then
				local names = {select(3, string.split(":", option))}
				for i = 1, #names, 4 do
			...
			elseif value == "ClickSets" then
				if arg1 == DB.MyClass then
					...
					local results = {select(4, string.split(":", option))}
					for i = 1, #results, 2 do
						results[i+1] = string.gsub(results[i+1], "`", ":")
						results[i+1] = string.gsub(results[i+1], "}", ";")
