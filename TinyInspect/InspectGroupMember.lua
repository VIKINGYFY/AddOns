...
local function GetButton(parent, index)
	if (not parent["button"..index]) then
		...
		button.ilvl = B.CreateFS(button, 14, "", "system", "LEFT", 24, -1)
		button.name = B.CreateFS(button, 14, "", false, "LEFT", 66, -1)
		button.spec = B.CreateFS(button, 14, "", false, "RIGHT", -8, -1)
...
local function UpdateProgress(numCurrent, numTotal)
	local value = ceil(numCurrent*100/max(1,numTotal))
